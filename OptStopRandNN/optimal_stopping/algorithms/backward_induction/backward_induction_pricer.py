...
def compute_input_dim(model, use_var, use_payoff_as_input, use_spot_as_input=True):
  """Compute the input feature dimension for regressors/networks.

  Rules:
  - spot features count = model.nb_stocks if `use_spot_as_input` True else 0
  - if use_var True, add model.var_dim if present else model.nb_stocks
  - if use_payoff_as_input True, add 1
  """
  spot_dim = model.nb_stocks if use_spot_as_input else 0
  var_dim = getattr(model, 'var_dim', model.nb_stocks)
  return spot_dim + (var_dim if use_var else 0) + (
      1 if use_payoff_as_input else 0)
...
class AmericanOptionPricer:
  ...
  def price(self, train_eval_split=2):
    ...
    # prepare stopping time recorder for evaluation (test) half
    # initialize with terminal date index (model.nb_dates)
    tau = np.full(stock_paths.shape[0], fill_value=model.nb_dates, dtype=int)
    ...
    for i, date in enumerate(range(stock_paths.shape[2] - 2, 0, -1)):
      ...
      # record stopping time for evaluation paths (first stopping date)
      if np.any(stopping_rule[self.split:]):
        idxs = np.where((stopping_rule > 0.5) & (np.arange(len(stopping_rule)) >= self.split) & (tau == model.nb_dates))[0]
        if idxs.size:
          tau[idxs] = date
    ...
    # write tau values for evaluation half to output CSV
    #try:
      #out_dir = os.path.join(os.path.dirname(__file__), "../../output/metrics_draft")
      #os.makedirs(out_dir, exist_ok=True)
      #tau_fpath = os.path.join(out_dir, f"{int(time.time()*1000)}_tau.csv")
      #with open(tau_fpath, "w") as f:
        #f.write("path_index,tau\n")
        #for i, t in enumerate(tau[self.split:], start=self.split):
        #  f.write(f"{i},{t}\n")
    try:
    ...
    unique, counts = np.unique(tau[self.split:], return_counts=True)
    summary = dict(zip(unique.tolist(), counts.tolist()))
    print(f"tau summary (eval half) counts by date: {summary}")
      #print(f"Written tau file: {tau_fpath}")
      # also write a compact stops CSV next to the run tmp-file prefix
      
    #except Exception as e:
    #  print(f"Warning: failed to write tau file: {e}")
