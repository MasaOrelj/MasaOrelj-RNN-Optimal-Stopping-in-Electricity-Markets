...
class ReservoirLeastSquares2(Regression):
  ...
  def calculate_regression(self, X_unsorted, Y, in_the_money, in_the_money_all,
    ...
    # robustly handle occasional off-by-one differences where calling code
    # already appended a bias column (ones) while coefficients were computed
    # without it (or vice-versa). Try the direct dot first, and on failure
    # attempt a sensible fallback by dropping the last column if it is ones.
    try:
      continuation_values = np.dot(reg_input[in_the_money_all[0]], coefficients[0])
    except ValueError:
      # fallback: if reg_input has an extra final ones column, try without it
      if reg_input.shape[1] == coefficients[0].shape[0] + 1 and \
         np.allclose(reg_input[:, -1], 1.0):
        continuation_values = np.dot(reg_input[in_the_money_all[0], :-1], coefficients[0])
      else:
        # re-raise for unexpected mismatches
        raise
