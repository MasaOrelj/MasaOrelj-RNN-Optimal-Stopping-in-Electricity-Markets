...
#    self.state_size = self.input_dim + 2
...
class FQI_ReservoirFast(FQI.FQIFast):
  ...
  def evaluate_bases_all(self, stock_price):
    ...
    # Ensure reservoir input size matches current data (lazy init / reinit)
    try:
      curr_in = self.reservoir2.NN[0].in_features
    except Exception:
      curr_in = None
    expected_in = stocks.shape[-1]
    if curr_in is None or curr_in != expected_in:
      activation = getattr(self.reservoir2.NN, 'modules', None)
      try:
        act = self.reservoir2.NN[1]
      except Exception:
        act = None
      self.reservoir2 = randomized_neural_networks.Reservoir2(
        self.dim_out, expected_in, activation=act if act is not None else None)
...
#      (stocks.shape[0], 1)).unsqueeze(2)
#    stocks = torch.cat([stocks, time, 1-time], dim=-1)
#    # lazy re-init if input dim changed
#    try:
#      curr_in = self.reservoir2.NN[0].in_features
#    except Exception:
#      curr_in = None
...
#    # randomRNN expects linear in_features = state_size + hidden_size (or 2*hidden if extend)
#    try:
#      curr_in = self.reservoir2.NN[0].in_features
#    except Exception:
#      curr_in = None
#    expected_state = stocks.shape[-1]
#    expected_in = expected_state + getattr(self.reservoir2, 'hidden_size', self.dim_out)
#    if curr_in is None or curr_in != expected_in:
#      # re-init preserving factors and extend
#      factors = getattr(self.reservoir2, 'factors', (1., 1., 1.))
#      extend = getattr(self.reservoir2, 'extend', False)
#      self.reservoir2 = randomized_neural_networks.randomRNN(
#        hidden_size=self.dim_out, state_size=expected_state, factors=factors,
#        extend=extend)
