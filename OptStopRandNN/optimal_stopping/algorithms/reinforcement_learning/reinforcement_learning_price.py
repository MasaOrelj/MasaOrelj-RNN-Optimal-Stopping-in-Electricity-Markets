...
class FQI_RL(backward_induction_pricer.AmericanOptionPricer):
  ...
  def __init__(self, model, payoff, nb_epochs=20, nb_batches=None,
    ...
    # whether to include spot in regression input
    self.use_spot_as_input = use_spot_as_input
    # compute input dim without calling the pricer ctor
    self.input_dim = backward_induction_pricer.compute_input_dim(
      self.model, self.use_var, self.use_payoff_as_input, self.use_spot_as_input)
    self.nb_epochs = nb_epochs
    self.nb_base_fcts = 0
  ...
  def price(self):
    ...
    ex_dates = np.zeros(nb_paths, dtype=int)
    for path in range(nb_paths):
      for date in range(nb_dates):
        ...
        if payoff > continuation_value or (date == nb_dates-1):
          ...
          ex_dates[path] = date
          break
    # store exercise dates for downstream analysis
    try:
      self.last_ex_dates = ex_dates
      unique, counts = np.unique(self.last_ex_dates[self.split:], return_counts=True)
